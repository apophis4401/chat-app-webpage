<template style="margin: 0">
  <header style="background-color: burlywood; padding: 0; margin: 0">
    <button @click="logOut" style="margin: 1%">Выйти из системы</button>
  </header>
  <div class="chat">
    <div class="chatBox">
      <div
...
export default {
  ...
  methods: {
    ...
    sendMessage() {
      ...
      axios
        .post("http://localhost:4000/api/send-message", { user, message })
        .then(() => {
          this.newMessage = "";
          this.fetchMessages();
        })
    ...
    fetchMessages() {
      axios
        .get("http://localhost:4000/api/get-messages")
        .then((response) => {
